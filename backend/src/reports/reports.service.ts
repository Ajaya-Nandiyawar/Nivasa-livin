...
export class ReportsService {
  ...
  async getRevenue(filter: ReportFilterDto) {
    let query: any = this.db.selectFrom('payments')
      .select([
        sql<string>`to_char(date_trunc('month', payment_date), 'YYYY-MM')`.as('month'),
  ...
  async getOccupancy(filter: ReportFilterDto) {
    let bedQuery: any = this.db.selectFrom('beds')
      .select([
        sql<number>`COUNT(*)`.as('total_beds'),
        sql<number>`SUM(CASE WHEN status = 'OCCUPIED' THEN 1 ELSE 0 END)`.as('occupied_beds')
      ]);
  ...
  async getOutstanding(filter: ReportFilterDto) {
    let query: any = this.db.selectFrom('rent_records')
      .select([
        sql<number>`
  ...
  async getExpenses(filter: ReportFilterDto) {
    let query: any = this.db.selectFrom('expenses')
    ...
    if (filter.from_date) {
      query = query.where('expenses.expense_date', '>=', new Date(filter.from_date));
    }
    if (filter.to_date) {
      query = query.where('expenses.expense_date', '<=', new Date(filter.to_date));
    }
  ...
  async getTenantTurnover(filter: ReportFilterDto) {
    let moveInQuery: any = this.db.selectFrom('bookings')
      .select([
        sql<string>`to_char(date_trunc('month', check_in_date), 'YYYY-MM')`.as('month'),
        sql<number>`COUNT(*)`.as('move_ins')
      ])
      .where('deleted_at', 'is', null);

    let moveOutQuery: any = this.db.selectFrom('bookings')
      .select([
        sql<string>`to_char(date_trunc('month', check_out_date), 'YYYY-MM')`.as('month'),
    ...
    if (filter.property_id) {
      moveInQuery = moveInQuery.innerJoin('beds', 'beds.id', 'bookings.bed_id')
        .innerJoin('rooms', 'rooms.id', 'beds.room_id')
        .innerJoin('floors', 'floors.id', 'rooms.floor_id')
        .where('floors.property_id', '=', filter.property_id);
      
      moveOutQuery = moveOutQuery.innerJoin('beds', 'beds.id', 'bookings.bed_id')
        .innerJoin('rooms', 'rooms.id', 'beds.room_id')
        .innerJoin('floors', 'floors.id', 'rooms.floor_id')
        .where('floors.property_id', '=', filter.property_id);
    }
    ...
    if (filter.from_date) {
      moveInQuery = moveInQuery.where('check_in_date', '>=', new Date(filter.from_date));
      moveOutQuery = moveOutQuery.where('check_out_date', '>=', new Date(filter.from_date));
    }
    if (filter.to_date) {
      moveInQuery = moveInQuery.where('check_in_date', '<=', new Date(filter.to_date));
      moveOutQuery = moveOutQuery.where('check_out_date', '<=', new Date(filter.to_date));
    }

    const moveIns = await moveInQuery.groupBy(sql`date_trunc('month', check_in_date)`).execute();
    const moveOuts = await moveOutQuery.groupBy(sql`date_trunc('month', check_out_date)`).execute();

    // Merge in JS
    const turnoverMap: Record<string, { month: string, move_ins: number, move_outs: number }> = {};
    
    moveIns.forEach(item => {
      turnoverMap[item.month] = { month: item.month, move_ins: Number(item.move_ins), move_outs: 0 };
    });
    
    moveOuts.forEach(item => {
      if (!turnoverMap[item.month]) {
        turnoverMap[item.month] = { month: item.month, move_ins: 0, move_outs: Number(item.move_outs) };
      } else {
        turnoverMap[item.month].move_outs = Number(item.move_outs);
      }
    });

    return Object.values(turnoverMap).sort((a, b) => a.month.localeCompare(b.month));
  }

  async exportRentRecords(filter: ReportFilterDto) {
    const query = this.db.selectFrom('rent_records')
      .innerJoin('bookings', 'bookings.id', 'rent_records.booking_id')
  ...
  async exportTenants(filter: ReportFilterDto) {
    const query = this.db.selectFrom('tenants')
      .select([
